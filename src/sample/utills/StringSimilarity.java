...
/**
 * Created by Jan on 21/03/2015.
 */
public class StringSimilarity {

    public double similarity_common_words(String x, String y) {
        String[] xwords = x.toLowerCase().split(" "); //[^a-z]+
        String[] ywords = y.toLowerCase().split(" ");
        Set<String> xset = new HashSet<String>(Arrays.asList(xwords));
        Set<String> yset = new HashSet<String>(Arrays.asList(ywords));
        xset.retainAll(yset);

        int minn = Math.min(xwords.length, ywords.length);
        return (double)xset.size()/minn;
    }
